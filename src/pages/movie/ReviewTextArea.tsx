...
export const ReviewTextArea = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditMode && myReview) {
        ...
        console.log('[ReviewTextArea] 리뷰 생성 요청:', {
          tmdbId,
          star: rating,
          content: value,
          spoiler: isSpoiler,
  ...
  return (
    <ReviewInputContainer>
      <ReviewInputTitle>
        {myReview ? (isEditMode ? '리뷰 수정' : '내 리뷰') : '리뷰 작성'}
      </ReviewInputTitle>
      <ReviewInputForm onSubmit={handleSubmit}>
        ...
        <FormFooter>
          <CharCount>
            {isEditMode ? value.length : (myReview?.content?.length ?? 0)} / 500자
          </CharCount>
          <ButtonWrapper>
            <FormControlLabel
            ...
            />
            {myReview && (
              <TextareaButton size="small" onClick={handleDelete}>
                삭제
              </TextareaButton>
            )}
            {isEditMode ? (
              <TextareaButton size="small" disabled={isSubmitting} onClick={handleSubmit}>
                저장
              </TextareaButton>
            ) : (
              <TextareaButton
